...
import '../../services/phase3_final_test_service.dart';
import '../../services/gating_service.dart';
import '../../../../core/utils/error_handler.dart';
import '../../../../core/constants/app_config.dart';

/// Provider for managing Phase 3 Final Test state
...
class Phase3FinalTestProvider extends ChangeNotifier {
  ...
  Phase3FinalTestProvider({
    required Phase3FinalTestService testService,
  ...
  /// Get total number of questions in the test (always 30)
  ...
  /// Loads questions from all Phase 3 lessons and prepares the test
  ...
  Future<void> startTest({int retryCount = 0}) async {
    ...
    try {
      ...
      if (!canTake) {
        _error = 'Please master all Phase 3 lessons before taking the final test';
        _isLoading = false;
        notifyListeners();
      ...
      // Validate minimum question count (at least 24 questions required)
      if (_questions.length < 24) {
        _error = 'Insufficient questions available (${_questions.length}/30). Please complete more lessons.';
        _isLoading = false;
        notifyListeners();
        throw Exception(_error);
      }
      ...
      print('Phase 3 test started successfully with ${_questions.length} questions');
    } catch (e, stackTrace) {
      ErrorHandler.logError('Phase3FinalTestProvider.startTest', e, stackTrace);
  ...
  Future<void> retrySubmitTest() async {
    if (_testResult != null) {
      ...
      try {
        ...
        ErrorHandler.logError('Phase3FinalTestProvider.retrySubmitTest', e);
  ...
  Future<void> submitTest() async {
    ...
    try {
      ...
      try {
        ...
        ErrorHandler.logError('Phase3FinalTestProvider.submitTest - Calculation', e);
      ...
      try {
        await _testService.saveTestResult(_testResult!);
        print('Phase 3 test result saved successfully');
        // Clear any previous error since save succeeded
        _error = null;
      } catch (storageError) {
        // Log error but don't fail the submission
        ErrorHandler.logError('Phase3FinalTestProvider.submitTest - Storage', storageError);
      ...
      ErrorHandler.logError('Phase3FinalTestProvider.submitTest', e, stackTrace);
  ...
  Future<void> loadPreviousResult() async {
    ...
    try {
      ...
      if (_testResult == null) {
        print('No previous Phase 3 test result found');
      } else {
        print('Loaded previous Phase 3 test result: ${_testResult!.correctAnswers}/${_testResult!.totalQuestions}');
      }
      
      _isLoading = false;
      notifyListeners();
    } catch (e, stackTrace) {
      ErrorHandler.logError('Phase3FinalTestProvider.loadPreviousResult', e, stackTrace);
      _error = ErrorHandler.getUserMessage(e);
      _isLoading = false;
      notifyListeners();
  ...
  Future<bool> hasPassedBefore() async {
    try {
      ...
      ErrorHandler.logError('Phase3FinalTestProvider.hasPassedBefore', e);
      return false;
    }
  }
  ...
  /// Check if all Phase 3 lessons are mastered
  ...
  Future<bool> canTakeTest() async {
    ...
    if (_gatingService != null) {
      try {
        return await _gatingService.isFinalTestAccessible(3);
      } catch (e) {
    ...
    try {
      return await _testService.areAllPhase3LessonsMastered();
    } catch (e) {
      ErrorHandler.logError('Phase3FinalTestProvider.canTakeTest', e);
      return false;
    }
  }
  ...
  Future<int?> getLastTestScore() async {
    try {
      ...
      ErrorHandler.logError('Phase3FinalTestProvider.getLastTestScore', e);
      return null;
    }
  }
